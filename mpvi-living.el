...
(cl-defmethod mpvi-extract-url ((_ (eql :huya.com)) url &rest _)
  ...
  (let* ((info (mpvi-huya-room-info url))
         ...
         (url (if (= 1 (length urls)) (car urls)
                (completing-read "Url: " urls nil t))))
    (list :url url :title title :logo "虎牙")))

(cl-defmethod mpvi-extract-playlist ((_ (eql :huya.com)) &rest _)
  "No need to check playlist for huya link." nil)
...
(cl-defmethod mpvi-extract-url ((_ (eql :douyu.com)) url &rest _)
  ...
  (let* ((info (mpvi-douyu-room-info url))
         ...
         (url (if (= 1 (length urls)) (car urls)
                (completing-read "Url: " urls nil t))))
    (list :url url :title title :logo "斗鱼")))

(cl-defmethod mpvi-extract-playlist ((_ (eql :douyu.com)) &rest _)
...
(cl-defmethod mpvi-extract-url ((_ (eql :douyin.com)) url &rest _)
  ...
  (when (string-match-p "live" url)
    (let* ((info (mpvi-douyin-room-info url))
           ...
           (url (if (= 1 (length urls)) (car urls)
                  (completing-read "Url: " urls nil t))))
      (list :url url :title title :logo "抖音"))))

(cl-defmethod mpvi-extract-playlist ((_ (eql :douyin.com)) &rest _)
