...
最近几天将其翻新了一下。因为发现用它来拉片、看视频，简直不要太爽!
...
- [biliass](https://github.com/yutto-dev/biliass)，安装这个后，看 B 站视频就有弹幕啦

对于 Arch Linux 用户:
```sh
yay -S mpv yt-dlp ffmpeg tesseract biliass-git xclip
```
...
4. `mpvi-export`，借助 ffmepg/ytdlp 实现视频的下载、剪辑、转码
...
- 如果纯粹看视频，并通过 minibuffer 操控 MPV，并不需要 Org Mode。但进行插入、编辑操作则必须在 Org Mode Buffer 中
