...
(defvar mpvi-ffmpeg-option-tpls
  `(("-ss 0"         . "seeks (in/out)")
    ...
    "-map 0:a"
    "-map 0:s"
    ("-minrate 964K -maxrate 3856K -bufsize 2000K" . "trans-rating")
    ("-vf transpose=2"  . "transpose")
    ("-vf scale=480:-1" . "trans-sizing")
    ("-vf scale=iw/2:ih/2" . "trans-sizing")
    ("-af volume=30dB" . "trans-volume")
    (,(format "-vf %s" mpvi-ffmpeg-gif-filter) . "gen gif")
    ("-c:v libx264 -crf 28 -c:a copy" . "used for compress")
    ("-vn -c:a copy -b:a 192k" . "pick only the audio")
    ("-ss 00:00:05 -vframes 1 -q:v 2" . "screenshot")
    ("-i watermark.png -filter_complex \"overlay=W-w-10:10\"" . "add watermark"))
...
This can be used to cut/resize/reformat and so on."
  ...
  (let* ((beg (if (numberp beg) (format " -ss %s" beg)))
    ...
    (cl-labels ((bs ()
                ...
                (snippet ()
                  (interactive)
                  (insert (mpvi--read-cmd-option "Insert ffmpeg option: " mpvi-ffmpeg-option-tpls)))
                (quit ()
                  (interactive)
                  (state help-tips)
...
(defun mpvi-ytdl-exec (url &optional target beg end)
  ...
  (let* ((meta (mpvi-ytdl-dump-url url))
    ...
    (cl-labels ((bs ()
                ...
                (snippet ()
                  (interactive)
                  (insert (mpvi--read-cmd-option "Insert yt-dlp option: " mpvi-ytdl-option-tpls)))
                (quit ()
                  (interactive)
                  (state nil)
                  (kill-buffer-and-window))
