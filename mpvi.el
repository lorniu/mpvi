...
(defvar mpvi-favor-cmds
  '((cycle pause)
    ...
    playlist-prev playlist-next ab-loop
    seek revert-seek
    (screenshot "video")
...
(cl-defun mpvi-play (path &optional (beg 0) end type noseek)
  "Play PATH from BEG to END.
TYPE is a flag, emms for played from emms and append for add to playlist.
When NOSEEK is not nil then dont try to seek but open directly."
  ...
  (if (and (not noseek) (emms-player-mpv-proc-playing-p) (equal path (mpvi-origin-path)))
      ...
      (when (mpvi-seekable)
        ...
        (mpvi-set 'time-pos beg)
        (mpvi-set 'pause 'no))
    ;; If path is not the current playing, load it
    (let ((ver<38 (ignore-errors (mpvi-compare-mpv-version #'< '(0 38 0))))
          (living (emms-player-mpv-proc-playing-p))
          logo title subfile load-opts start-func)
      (unless (eq type 'emms) (message "Waiting %s..." path))
      ...
      (when (mpvi-url-p path)
        (when-let* ((ret (mpvi-extract-url nil path)))
          ...
          (setq load-opts (plist-get ret :load-opts))
          (setq start-func (plist-get ret :start-func))))
      (let* ((load-opts (mapconcat (lambda (x) (format "%s=%s" (car x) (cdr x)))
                                   (append `((start . ,beg))
             ...
             (load-handler (lambda (_ err)
                             ...
                             (if start-func
                                 (funcall start-func)
                               (message "%s" (if title
                                                 (concat (if logo (concat "/" logo)) ": " (propertize title 'face 'font-lock-keyword-face))
                                               "")))))
             (play-cmd `(batch
                         ,@(unless (or living (eq type 'emms))
                         ...
                         ((loadfile ,path
                                    ,(if (eq type 'append) 'append 'replace)
                                    ,@(if ver<38 (list load-opts) (list -1 load-opts)))
                          . ,load-handler)))
             (start-func (lambda ()
...
(defun mpvi-set (property val)
  ...
  (interactive (let* ((_ (mpvi-check-live))
                      (prop (intern
                             (completing-read "Change Property: "
                                              (lambda (input pred action)
                                                (if (eq action 'metadata)
                                                    `(metadata (display-sort-function . ,#'identity))
...
(defun mpvi-open (path)
  "Play PATH with MPV player.
PATH is a local video or remote url. Prefer the one at point."
  (interactive (catch 'mpvi-open
                 (list (mpvi-read-file-or-url "Play video (file or url): " (mpvi-emms-video-regex)))))
...
;;;###autoload
(defun mpvi-emms-add (path)
  "Add PATH to EMMS playlist."
  ...
  (if (mpvi-url-p path)
      (let ((playlist (mpvi-extract-playlist
        ...
        (cl-loop with desc = (read-string "Label for current url: " (car playlist))
                 for url in choosen
                 for title = (if (> (length desc) 0) (format "%s - %s" desc url) url)
                 do (let ((track (emms-track 'mpvi url)))
                      (emms-track-set track 'info-title title)
                      (with-current-emms-playlist
                        (emms-playlist-insert-track track)))))
    (setq path (expand-file-name path))
    (cond ((file-regular-p path)
           (let ((track (emms-track 'mpvi path)))
             (with-current-emms-playlist (emms-playlist-insert-track track))))
          ((file-directory-p path)
           (cl-loop with regex = (emms-player-get emms-player-mpv 'regex)
                    with files = (if current-prefix-arg
                                     (when (y-or-n-p (format "Recusively search and add (%s)?" path))
                                       (emms-source-file-directory-tree path regex))
                                   (directory-files path t regex))
                    for file in files
                    unless (or (let ((case-fold-search nil))
                                 (string-match emms-source-file-exclude-regexp file))
                               (file-directory-p file))
                    do (let ((track (emms-track 'mpvi (expand-file-name file))))
                         (with-current-emms-playlist (emms-playlist-insert-track track)))))
