...
(defcustom mpvi-ytdlp-extra-options nil
...
(defvar mpvi-last-save-directory emms-source-file-default-directory)
      ...
      (let* ((lowp (ignore-errors (mpvi-compare-mpv-version #'< '(0 38 0))))
        ...
        (if (and item (equal item current)) (funcall seek)
          (let* ((items (if item `((,item)) items))
                 ...
                 (ytdl-opt (when mpvi-ytdlp-extra-options
                             (mapconcat #'identity
                                        (cl-loop with options = (cl-copy-list mpvi-ytdlp-extra-options)
                                                 while options
...
(cl-defmethod mpvi-extract ((_ (eql 'url)) url &rest _)
  ...
  (or (cdr (assoc url mpvi-url-info-cache))
      (let* ((json (mpvi-ytdlp-dump-url url))
             (info (list :origin url
                         :title (alist-get 'title json)
                         :up (alist-get 'uploader json))))
            ...
            (setq info
                  `(,@info :items
                           ,(cl-loop for item across (alist-get 'entries json)
                                     ...
                                     for up = (alist-get 'uploader item)
...
(defvar mpvi-ytdlp-default-format "bv*+ba/b"
...
(defun mpvi-ytdlp-dump-url (url &optional opts)
  "Return metadata of URL, pass extra OPTS to `yt-dlp' for querying."
  (unless (executable-find "yt-dlp")
    ...
    (condition-case err
        (progn
          ...
          (apply #'mpvi-call-process
                 "yt-dlp" url "--dump-single-json" "--flat-playlist" "--no-warnings" (or opts mpvi-ytdlp-extra-options))
          (goto-char (point-min))
          (json-read))
...
Pass extra OPTS to mpv if it is not nil."
  ...
  (with-temp-buffer
    ...
    (apply #'mpvi-call-process "yt-dlp" url "--print" field (or opts mpvi-ytdlp-extra-options))
    (goto-char (point-min))
    (if (re-search-forward "^yt-dlp: error:.*$" nil t)
        (user-error "Error to get `yt-dlp' template/%s: %s" field (match-string 0))
      (string-trim (buffer-string)))))

(defun mpvi-ytdlp-pick-format (url &optional opts)
  "Completing read the video formats for URL through `yt-dlp'.
OPTS is the extra options passed to the program.
Return (suggestion-save-name . video-format)."
  (unless (executable-find "yt-dlp")
    (user-error "Program 'yt-dlp' should be installed"))
  (with-temp-buffer
    (apply #'mpvi-call-process "yt-dlp" url "--list-formats" (or opts mpvi-ytdlp-extra-options))
    (goto-char (point-min))
    (unless (and (re-search-forward "Available formats for \\(.+\\):" nil t)
    ...
    (let* ((name (if (equal (mpvi-get 'path) url)
                   ...
                   (match-string 1)))
           (items (cl-loop with lines = (split-string (string-trim (buffer-substring (point) (point-max))) "\n")
           ...
           (fmts (completing-read-multiple
                  (format "Format (Multiple, like '12,76' or 'bv,ba'. Default: %s): " mpvi-ytdlp-default-format)
                  (lambda (input pred action)
                    (if (eq action 'metadata)
                        `(metadata
                  ...
                  nil nil nil nil mpvi-ytdlp-default-format))
           (ext (if-let* ((id (cl-find-if (lambda (c) (assoc c items)) fmts :from-end t)))
                    (cadr (caddr (assoc id items)))
                  "mp4"))
           (target (thread-last
                     (format "%s_%s_%s.%s" name (string-join fmts "+") (truncate (float-time)) ext)
                     (string-replace "/" "-"))))
      (cons target (string-join fmts "+")))))
...
OPTS is a string, pass to `yt-dlp' when it is not nil."
  ...
  (let* ((fmt (mpvi-ytdlp-pick-format url opts))
         (beg (if (numberp beg) (format " -ss %s" beg)))
         (end (if (numberp end) (format " -to %s" end)))
         (extra (mapconcat (lambda (c) (format " %s" c)) (or opts mpvi-ytdlp-extra-options) " "))
...
Pass OPTS to `yt-dlp' when it is not nil."
  ...
  (with-temp-buffer
    ...
    (apply #'mpvi-call-process
           ...
           (or opts mpvi-ytdlp-extra-options))
    (goto-char (point-min))
